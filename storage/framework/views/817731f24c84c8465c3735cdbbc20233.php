<?php $__env->startSection('title', __('Form questions')); ?>
<?php $__env->startSection('heading', __('Form questions')); ?>
<?php $__env->startSection('subheading', __('These appear on the public complaint form, in order. English and Bangla labels are shown together when both are set.')); ?>

<?php $__env->startSection('content'); ?>
    <div class="mb-6 flex items-center justify-between">
        <a href="<?php echo e(route('admin.settings.questions.create')); ?>" class="inline-flex items-center rounded-xl bg-slate-900 text-white px-5 py-2.5 text-sm font-semibold hover:bg-slate-800 shadow-lg shadow-slate-900/10 transition"><?php echo e(__('Add question')); ?></a>
        ...
        <?php $__currentLoopData = $questions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $q): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <div class="rounded-2xl bg-white border border-slate-200 p-5 shadow-sm flex items-center justify-between gap-6 hover:border-emerald-200/60 transition">
                <div class="min-w-0 flex-1">
                    <p class="font-semibold text-slate-900"><?php echo e($q->prompt); ?></p>
                    <?php if(filled($q->prompt_bn)): ?>
                        <p class="text-sm text-slate-600 mt-1 noto-bn" lang="bn"><?php echo e($q->prompt_bn); ?></p>
                    <?php endif; ?>
                    <p class="text-xs text-slate-500 mt-2">
                        <?php echo e(__('Type')); ?>:
                        <?php if($q->type === \App\Models\ComplaintQuestion::TYPE_BOOLEAN): ?>
                            <?php echo e(__('Checkbox')); ?>

                        <?php elseif($q->type === \App\Models\ComplaintQuestion::TYPE_FILE): ?>
                            <?php echo e(__('File')); ?>
                        ...
                        <?php endif; ?>
                        <?php if($q->type === \App\Models\ComplaintQuestion::TYPE_SELECTION && $q->allow_multiple): ?>
                            (<?php echo e(__('multiple choice')); ?>)
                        <?php endif; ?>
                        · <?php echo e($q->is_active ? __('Active') : __('Inactive')); ?>

                    </p>
                    <?php if($q->options->isNotEmpty()): ?>
                        <ul class="text-sm text-slate-600 mt-3 space-y-1 pl-4 list-disc">
                            <?php $__currentLoopData = $q->options; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $o): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <li><?php echo e($o->label); ?></li>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        </ul>
                    <?php endif; ?>
                </div>
                <div class="flex items-center gap-2 shrink-0">
                    <a href="<?php echo e(route('admin.settings.questions.edit', $q)); ?>" class="inline-flex h-8 items-center rounded-lg border border-slate-200 bg-white px-3 text-xs font-bold text-slate-700 shadow-sm transition hover:bg-slate-50 hover:border-slate-300">
                        <?php echo e(__('Edit')); ?>

                    </a>
                    <button @click="openModal('<?php echo e(route('admin.settings.questions.destroy', $q)); ?>')" type="button" class="inline-flex h-8 items-center rounded-lg border border-red-100 bg-red-50 px-3 text-xs font-bold text-red-600 shadow-sm transition hover:bg-red-100 hover:border-red-200">
    ...
    <div x-show="showModal" 
        ...
        <div x-show="showModal" 
            ...
            <div class="mb-6 flex flex-col items-center text-center">
                ...
                <p class="mt-2 text-sm font-bold text-slate-400 uppercase tracking-widest"><?php echo e(__('Irreversible action')); ?></p>
            </div>
            ...
            <div class="mb-8 rounded-2xl bg-slate-50 p-5 border border-slate-100 text-center">
                <p class="text-[13px] font-medium leading-relaxed text-slate-600">
                    <?php echo e(__('Are you sure you want to remove this question? While historical data is preserved, it will be hidden from the active form.')); ?>
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /home/storage/517/4794517/user/htdocs/resources/views/admin/settings/questions/index.blade.php ENDPATH**/ ?>