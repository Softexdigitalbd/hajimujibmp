...
@section('subheading', __('Updating labels changes what visitors see; stored answers keep their previous text.'))
    ...
    <div class="relative -mx-6 lg:-mx-8 -mt-6 lg:-mt-8 mb-0 min-h-[calc(100vh-6rem)] overflow-hidden rounded-2xl border border-slate-200/60 bg-gradient-to-br from-slate-50 via-white to-teal-50/35">
        ...
        <div class="relative max-w-3xl mx-auto px-6 lg:px-10 py-10 lg:py-14">
            <div class="flex items-start justify-between gap-4 mb-10">
                <div>
                    <p class="text-xs font-semibold uppercase tracking-widest text-teal-700/80">{{ __('Editing') }} #{{ $question->id }}</p>
                    <h2 class="mt-2 text-2xl font-bold text-slate-900 tracking-tight line-clamp-2">{{ $question->prompt }}</h2>
                </div>
                <a href="{{ route('admin.settings.questions.index') }}" class="shrink-0 text-sm font-semibold text-slate-500 hover:text-slate-800 transition">{{ __('Close') }}</a>
            </div>

            <form method="POST" action="{{ route('admin.settings.questions.update', $question) }}" 
                x-data="{ 
                    type: '{{ old('type', $question->type) }}',
                    options: @js(old('options', $question->options->pluck('label')->all() ?: ['']))
                }"
                ...
                @method('PUT')
                <div>
                    <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wide mb-2">{{ __('Label (English)') }}</label>
                    <input type="text" name="prompt" value="{{ old('prompt', $question->prompt) }}" class="w-full rounded-xl border-2 border-slate-200 bg-white py-3 px-4 text-sm font-semibold text-slate-800 focus:border-emerald-500 focus:ring-emerald-500/20 shadow-sm transition-all" required>
                </div>
                <div>
                    <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wide mb-2">{{ __('Extra Bangla line (optional)') }}</label>
                    <input type="text" name="prompt_bn" value="{{ old('prompt_bn', $question->prompt_bn) }}" lang="bn" class="w-full rounded-xl border-2 border-slate-200 bg-white py-3 px-4 text-sm font-semibold text-slate-800 focus:border-emerald-500 focus:ring-emerald-500/20 shadow-sm transition-all noto-bn">
                </div>
                <div>
                ...
                <div x-show="type === 'selection'" 
                    ...
                    <label class="flex items-center gap-4 text-sm font-bold text-slate-700 pt-3 cursor-pointer group border-t border-emerald-100/40">
                        <div class="relative flex h-6 w-6 shrink-0 items-center justify-center rounded-lg border-2 border-slate-300 bg-white transition-all group-hover:border-emerald-400 group-has-[:checked]:border-emerald-500 group-has-[:checked]:bg-emerald-500 group-has-[:checked]:shadow-lg group-has-[:checked]:shadow-emerald-500/20">
                            <input type="checkbox" name="allow_multiple" value="1" @checked(old('allow_multiple', $question->allow_multiple)) class="peer absolute h-full w-full opacity-0 cursor-pointer">
                ...
                <div class="grid sm:grid-cols-2 gap-x-6 gap-y-4">
                    <div>
                        ...
                        <input type="number" name="sort_order" value="{{ old('sort_order', $question->sort_order) }}" min="0" class="w-full rounded-xl border-2 border-slate-200 bg-white py-3 px-4 text-sm font-semibold text-slate-800 focus:border-emerald-500 focus:ring-emerald-500/20 shadow-sm transition-all">
                    </div>
                    <div class="flex flex-col justify-end gap-5 pb-1">
                        <label class="flex items-center gap-4 text-sm font-bold text-slate-700 cursor-pointer group">
                            <div class="relative flex h-6 w-6 shrink-0 items-center justify-center rounded-lg border-2 border-slate-300 bg-white transition-all group-hover:border-emerald-400 group-has-[:checked]:border-emerald-500 group-has-[:checked]:bg-emerald-500 group-has-[:checked]:shadow-lg group-has-[:checked]:shadow-emerald-500/20">
                                ...
                                <input type="checkbox" name="is_active" value="1" @checked(old('is_active', $question->is_active)) class="peer absolute h-full w-full opacity-0 cursor-pointer">
                                <svg class="h-3.5 w-3.5 scale-0 font-black text-white transition-transform peer-checked:scale-100" fill="none" stroke="currentColor" stroke-width="4" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M4.5 12.75l6 6 9-13.5" /></svg>
                            </div>
