...
@section('title', __('Complaint Journey'))
@section('heading', __('Complaint Journey Designer'))
@section('subheading', __('Design the step-by-step journey a complaint takes from submission to final resolution.'))

@section('content')
<div class="grid lg:grid-cols-[400px_1fr] gap-8 items-start" x-data="journeyArchitect()">

    {{-- Path Control Sidebar --}}
    <div class="space-y-6">
        <div class="rounded-3xl bg-white border border-slate-200 p-8 shadow-xl shadow-slate-200/40">
            <h3 class="text-[13px] font-black text-slate-900 uppercase tracking-widest mb-6">{{ __('Journey Builder') }}
            </h3>
            <div class="space-y-6">
                <div class="flex gap-4">
                    <div
                        ...
                        1</div>
                    <p class="text-[11px] leading-relaxed font-bold text-slate-500 uppercase tracking-tight">{{
                        __('Click a status node to select it as the source (highlights in indigo).') }}</p>
                </div>
                <div class="flex gap-4">
                    <div
                        class="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-indigo-50 font-black text-indigo-600 ring-1 ring-indigo-500/10 text-[11px]">
                        2</div>
                    <p class="text-[11px] leading-relaxed font-bold text-slate-500 uppercase tracking-tight">{{
                        __('Click a second node to instantly build a pathway between them.') }}</p>
                </div>
            </div>
            ...
            <div x-show="fromId" x-transition.opacity
                ...
                <div class="flex items-center justify-between mb-2">
                    <span class="text-[9px] font-black uppercase tracking-widest text-indigo-400">{{ __('Selected
                        Source') }}</span>
                    <button @click="fromId = ''; updateMermaid()"
                        class="text-[9px] font-black uppercase text-rose-400 hover:text-rose-600 tracking-widest">{{
                        __('Cancel') }}</button>
                </div>
        ...
        <div class="rounded-3xl bg-white border border-slate-200 p-8 shadow-xl shadow-slate-200/40">
            <div class="flex items-center justify-between mb-6 border-b border-slate-100 pb-4">
                <h3 class="text-[11px] font-black text-slate-900 uppercase tracking-widest">{{ __('Complain Journey') }}
                </h3>
                <span
                ...
                <button @click="resetAll()" x-show="transitions.length > 0"
                    class="text-[10px] font-black uppercase text-rose-500 hover:text-rose-600 tracking-widest underline underline-offset-2">{{
                    __('Reset All') }}</button>
            </div>
            ...
            <div class="mt-8">
                <form method="POST" action="{{ route('admin.settings.transitions.sync') }}">
                    @csrf
                    @method('PUT')
    ...
    <div
        ...
        <div class="px-8 py-6 border-b border-slate-100 bg-white flex items-center justify-between">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h2 class="text-sm font-black text-slate-900 uppercase tracking-widest">{{ __('Visual Journey Map')
                        }}</h2>
                    <p class="text-[10px] font-bold text-slate-400 uppercase tracking-tighter mt-0.5">{{ __('Real-time
                        Architect') }}</p>
                </div>
            </div>
            <div class="flex items-center gap-6">
                <div class="flex items-center gap-4">
                    <span class="flex items-center gap-2 text-[10px] font-black uppercase text-slate-400">
                        <span class="h-2.5 w-2.5 rounded-full bg-slate-100 border border-slate-300"></span> {{
                        __('Started') }}
                    </span>
                    <span class="flex items-center gap-2 text-[10px] font-black uppercase text-slate-400">
                        <span class="h-2.5 w-2.5 rounded-full bg-orange-100 border border-orange-300"></span> {{
                        __('Work') }}
                    </span>
                    <span class="flex items-center gap-2 text-[10px] font-black uppercase text-slate-400">
                        <span class="h-2.5 w-2.5 rounded-full bg-emerald-100 border border-emerald-300"></span> {{
                        __('Done') }}
                    </span>
                </div>
            </div>
        ...
        <div class="p-10 bg-slate-50/20 min-h-[600px] flex items-center justify-center relative select-none">
            ...
            <div
                ...
                <p class="text-[10px] font-black text-slate-400 uppercase tracking-[0.25em]">{{ __('Click nodes to draw
                    pathways') }}</p>
            </div>
        </div>
    </div>
...
<script>
    function journeyArchitect() {
        return {
            transitions: @js($statuses -> flatMap(fn($s) => $s -> transitionsFrom -> map(fn($t) => ['from' => $s -> id, 'to' => $t -> to_complaint_status_id, 'from_name' => $s -> alias_name, 'to_name' => $t -> toStatus -> alias_name ?? 'Undefined']))),
            statuses: @js($statuses -> map(fn($s) => ['id' => $s -> id, 'name' => $s -> alias_name, 'state' => $s -> state])),
            fromId: '',
            toId: '',
            ...
            resetAll() {
                if (confirm('{{ __('Are you sure you want to clear all connections ? ') }}')) {
                    this.transitions = [];
                    this.updateMermaid();
                }
            },
