...
@section('heading', __('Complaints'))
@section('subheading', __('Search, filter and manage submitted complaints.'))

@section('content')

{{-- Filters --}}
<div x-data="{ showFilter: {{ request()->anyFilled(['status_id', 'filters']) ? 'true' : 'false' }} }" class="mb-6">
    <div class="mb-4 flex justify-end">
        <button @click="showFilter = !showFilter" type="button"
            ...
            <span x-text="showFilter ? '{{ __('Hide filters') }}' : '{{ __('Show filters') }}'"></span>
    ...
    <form x-show="showFilter" x-transition method="GET"
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 items-end gap-5 p-5">
            <div class="min-w-[170px]">
                <label class="block text-[11px] font-bold uppercase tracking-wider text-slate-400 mb-2">{{ __('Status')
                    }}</label>
                <select name="status_id"
                    ...
                    <option value="{{ $st->id }}" @selected(request('status_id')==$st->id)>{{ $st->alias_name }}
                    </option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="min-w-[150px]">
                <label class="block text-[11px] font-bold uppercase tracking-wider text-slate-400 mb-2">{{ $fq->prompt
                    }}</label>
                @if($fq->type === \App\Models\ComplaintQuestion::TYPE_SELECTION)
                <select name="filters[{{ $fq->id }}]"
                    class="w-full rounded-xl border border-slate-200 bg-white py-2.5 px-3.5 text-sm shadow-sm focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 transition">
                    <option value="">{{ __('All') }} {{ $fq->prompt }}</option>
                    @foreach($fq->options as $opt)
                    <option value="{{ $opt->label }}" @selected(request("filters.{$fq->id}") == $opt->label)>{{
                        $opt->label
                        }}</option>
                    @endforeach
                </select>
                @elseif($fq->type === \App\Models\ComplaintQuestion::TYPE_BOOLEAN)
                <select name="filters[{{ $fq->id }}]"
                    class="w-full rounded-xl border border-slate-200 bg-white py-2.5 px-3.5 text-sm shadow-sm focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 transition">
                    <option value="">{{ __('Any') }}</option>
                    <option value="1" @selected(request("filters.{$fq->id}") === '1')>{{ __('Yes') }}</option>
                    <option value="0" @selected(request("filters.{$fq->id}") === '0')>{{ __('No') }}</option>
                </select>
                @else
                <input type="text" name="filters[{{ $fq->id }}]" value="{{ request('filters.'.$fq->id) }}"
                    placeholder="{{ $fq->prompt }}"
                    class="w-full rounded-xl border border-slate-200 bg-white py-2.5 px-3.5 text-sm shadow-sm focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 transition">
                @endif
            </div>
            @endforeach
            ...
            <div class="flex items-center justify-end gap-2 md:col-span-3 mt-2 border-t border-slate-100 pt-5">
                ...
                @if(request()->anyFilled(['status_id', 'filters']))
...
<div class="rounded-2xl border border-slate-200/60 bg-white shadow-sm overflow-hidden">
    ...
    <div
        ...
        <p class="text-xs font-bold text-slate-500">
            {{ $complaints->total() }} {{ Str::plural('complaint', $complaints->total()) }} found
        </p>
        @if(request()->anyFilled(['q', 'status_id', 'filters']))
        <span
    ...
    <table class="min-w-full text-sm">
        <thead>
            <tr class="border-b border-slate-100 text-left">
                <th class="px-6 py-3.5 text-[11px] font-bold uppercase tracking-widest text-slate-400">{{ __('ID') }}
                </th>
                <th class="px-6 py-3.5 text-[11px] font-bold uppercase tracking-widest text-slate-400">{{ __('Feedback
                    Subject') }}</th>
                <th class="px-6 py-3.5 text-[11px] font-bold uppercase tracking-widest text-slate-400">{{ __('Status')
                    }}</th>
                <th
                    class="hidden px-6 py-3.5 text-[11px] font-bold uppercase tracking-widest text-slate-400 sm:table-cell">
                    {{ __('Submitted') }}</th>
                <th class="px-6 py-3.5 text-[11px] font-bold uppercase tracking-widest text-slate-400 text-right">{{
                    __('Action') }}</th>
            </tr>
        </thead>
        <tbody class="divide-y divide-slate-50">
            ...
            <tr class="group transition-colors duration-200 hover:bg-emerald-50/30">
                ...
                <td class="px-6 py-4">
                    @php $sc = $c->status->colorClasses(); @endphp
                    <span
                ...
                <td class="hidden px-6 py-4 text-xs text-slate-500 sm:table-cell">
                    <span class="font-medium text-slate-600">{{ $c->created_at->format('M j, Y') }}</span>
                    <span class="ml-1 text-slate-400">{{ $c->created_at->format('h:i A') }}</span>
                </td>
            ...
            <tr>
                <td colspan="5" class="px-6 py-20 text-center">
                    <div class="flex flex-col items-center gap-4">
                        ...
                        <div>
                            <p class="text-sm font-bold text-slate-700">{{ __('No complaints found') }}</p>
                            <p class="mt-1 text-xs text-slate-400">{{ __('New submissions from the public form will
                                appear here.') }}</p>
                        </div>
                        @if(request()->anyFilled(['q', 'status_id', 'filters']))
                        <a href="{{ route('admin.complaints.index') }}"
