...
<body class="antialiased bg-gradient-to-br from-slate-50 via-white to-[#edf4f1]/30 text-slate-900 min-h-screen flex flex-col">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6">
            <div class="flex h-16 items-center justify-between">
                ...
                <nav class="flex items-center gap-1 text-sm font-medium">
                    <a href="{{ route('home') }}" class="rounded-lg px-3.5 py-2 text-slate-600 transition-all hover:bg-slate-100 hover:text-slate-900">{{ __('Home') }}</a>
                    <a href="{{ route('complaint.create') }}" class="rounded-md bg-[#2d6a4f] px-5 py-2 text-white font-semibold transition-colors hover:bg-[#143c30]">{{ __('Complaint') }}</a>
    ...
    <footer class="border-t border-slate-200/60 bg-white/50 backdrop-blur-sm">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 py-8 flex flex-col sm:flex-row items-center justify-between gap-4">
            <p class="text-sm text-slate-500">&copy; {{ date('Y') }} {{ config('app.name') }}. {{ __('All rights reserved.') }}</p>
            <div class="flex items-center gap-4 text-sm text-slate-400">
                <a href="{{ route('home') }}" class="transition hover:text-[#1b4d3e]">{{ __('Home') }}</a>
                <span class="text-slate-300">·</span>
                <a href="{{ route('complaint.create') }}" class="transition hover:text-[#1b4d3e]">{{ __('Submit complaint') }}</a>
            </div>
        </div>
    </footer>
</body>
